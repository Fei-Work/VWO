...
int main(int argc, char **argv)
{
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        string img_path = string(argv[3])+"/image/stereo_left/"+vstrImageFilenames[ni];
        im = cv::imread(img_path,CV_LOAD_IMAGE_UNCHANGED);
...
// ? [compiled][with][c11]此为查询是否遵循C11原则
...
void LoadImages(const string &strFile, vector<string> &vstrImageFilenames, vector<double> &vTimestamps)
{
    ...
    while(!f.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            // sRGB = std::to_string(t);
            // size_t dotPos = sRGB.find('.');
            // if (dotPos != std::string::npos) {
            //     sRGB = sRGB.substr(0, dotPos);
            // }
            sRGB = s + ".png";
