 ...
 */
void Optimizer::LocalBundleAdjustmentWithWheel(KeyFrame *pKF, bool* pbStopFlag, Map* pMap)
{
    ...
    // Step4. 将所有mp点对应的关键帧（除了前面加过的）放入到固定组里面，后面优化时不改变
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        if (pKFi->mpWheelPreintegrated)
        {
            ...
            if (!VP1 || !VP2)
            {
                cerr << "Error " << VP1 << ", " << VP2 << endl;
                continue;
            }
            vei[i] = new EdgeInertial(pKFi->mpWheelPreintegrated);
            vei[i]->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VP1));
            vei[i]->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VP2));
    ...
    for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end(); lit != lend; lit++)
    {
        ...
        vPoint->setId(id);
        // 因为使用了LinearSolverType，所以需要将所有的三维点边缘化掉，不太理解
        vPoint->setMarginalized(true);  
        optimizer.addVertex(vPoint);
